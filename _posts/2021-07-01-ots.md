...
因此, 数据量大之后, MySQL表只能当作一个SQL语法访问的单机KV来用.

拍脑袋的经验: 单表(单分区)数据量超过100GB, 或超过1kw行后, 需要特别注意读写模式是否足够简单, 是否有慢查询.
...
产品思路是面向客户的MySQL通讯协议不变, 从而最简化上车成本, 后面具体内部实现逻辑慢慢换掉. 
...
- 基于时间戳的版本控制和数据淘汰
- 支持自增ID相关功能
- MySQL的binlog功能对于我们数据同步格外重要, OTS有对应的通道服务, 并且按照分区键并行化
...
可以达到类似的节约宽表结构设计导致的空间浪费, 以及不方便加字段的弊端. 甚至还可以对任意字段查询走索引.