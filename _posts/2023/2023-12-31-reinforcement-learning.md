...
四要素
- policy: f: sensation -> action
- reword signal: 短期奖惩 (及时行乐) / 容易测量 
- value function: 长期目标/价值函数 (延迟满足) / 难以评估, 基于reward算出
- model: 环境运作的机制

与监督学习区别:
监督学习, 通过对于训练集归纳, 预测未见数据.
相当于给了每种棋局的最优解法, 但是缺少学习体和机制的交互过程.
实际问题上可训练的标注数据很少, 需要学习体自己尝试, 生成数据, 自学成才.

与无监督学习区别: RL目标是最大化奖赏, 无监督学习旨在发现背后的结构. 当然理解世界运作机制有助于最大化奖赏
...
废话一句, 这里每个赌博机的奖励分布函数未知, 知道了, 相当于上帝视角了, 拿着期望最大的使劲薅即可.
...
- A / action / 行为
- R / reward / 奖励, 数值
- 简化讨论当作离散问题表述, 即S/A/R可枚举, 实际连续问题也离散化去近似
- 每一步的行为导致奖励以及状态的变化
- d / dynamics / 环境运作机制 / `P(St, Rt | St-1, At-1)`
  - 状态转移概率 `P(St|St-1,At-1)`
  - 奖励概率 `P(Rt|St-1,At-1)`
  - 条件奖励概率 `P(Rt|St,St-1,At-1)`
  - 可以是转移概率或者确定的转移机制, 即值域为{0,1}
  - `d(s1,s2,a,r) -> [0,1] for s1 in S for s2 in S for a in A for r in R`
  - HMM比较: 显变量(奖励)完全基于隐变量(当前状态)表达, `p(s1,s2,a,r) = p(s1,s2,a) * p(s2,r)`
  - 对于学习体而言, p不一定是可知的, 但是状态S一定是要当作可观测的, 或者换个说法, S是学习体的观测/感知, 而不是实际世界运作的底层变量
- p / policy / 行动策略 / `P(At|St)`
  - `p(a,s) -> [0,1] for a in A for s in S`
  - 同样的, 可以是决策概率或者确定的决策机制
  - 对于确定性的策略, 可以通过表格或者字典的方式表征

下棋例子: S完全确定的, R是否获胜, A落子, p=对手决策, x=己方决策
...
MDP模型下的问题表述: 给定当前状态s, 求最优策略, 以及对应的最大价值 
...
对比标准优化问题表述: 这里参数是函数族(p), 而不是确定的参数, 因为标准优化问题对于策略结构做咯明确的假设

对于最大价值函数, 类似的, 有Bellman方程表示 (Bellman optimality equation)
...
即表述了当前状态的最大价值和下一步每个状态最大价值的关系, 同样也是可唯一求解的.
区别在于是否是对于策略函数p的泛函表示, 这里脱掉了p

解得v(s), 自然知道了最优策略 (Q: WHY)
...
1. 给定策略p, 评估v / policy evaluation
2. 基于评估v, 更新p / policy improvement
...
```
def get_v(p):
  v = {s: 0 if terminal(s) 1 for s in S}  // 价值函数单位初始化, 终止态价值为0 
  while True:
    e = 0  // 当前迭代更新误差
    for s in S:
...
这里每一轮重新估计v, 实际上是不需要的, 因为拿到的最大v等效拿到了最优策略, 因此上面的迭代argmax换成max后迭代即可
...
缺点: 每一轮遍历状态空间费时 `for s in S`
办法: 偷鸡, 每一轮只访问一部分状态做更新

DP算法针对问题大小是多项式时间复杂度的, 尽管策略空间是|S|**|A|

这里DP是针对MDP问题形式的讨论, 广义算法上常提的动态规划对于问题要求: 
- optimal structure: 父问题最优解是子问题最优解的组合, 不能保证收敛到最优解, 只能叫贪心解
  - 正例: 最短路径算法/A*
  - 反例: 背包问题
...
2. 每轮采样计算和最终步长有关, 和状态维度无关
3. 可以选择从指定开始的状态去模拟, 而不用计算所有状态的价值, 更实用
4. 不强求问题的马尔科夫性
...
这里习得的是一个确定性的函数.
如果引入了EE, 采用e贪心策略, 即决策函数以一定小概率随机, 其他情况遵循当前最优行动, 则整个过程可以去掉第一步的乱走. 伪码略.

on-policy VS off-policy: 生成样本数据的策略和优化的目标策略是否要求同一个; 人是否能完全自学顿悟, 还是需要借助他人知识.
从定义来说, off-policy包含on-policy, 自然更为强大, 学下棋给不给棋谱; on-policy简单, 收敛更快.
对于上文e贪心策略而言, 属于on-policy, 收敛到一个仍然保留探索可能性的策略; 对于off-policy, 则可能没有探索步骤, 而是一个确定性的策略.

first visit VS every visit: 观测序列里面是否允许包含当前预测态, every visit方式估算有偏差, 但是更加简单推荐.
...
> Zermelo's theorem: 在二人的有限游戏中, 如果双方皆拥有完全的资讯, 并且运气因素并不牵涉在游戏中, 那先行或后行者当中必有一方有必胜/必不败的策略
