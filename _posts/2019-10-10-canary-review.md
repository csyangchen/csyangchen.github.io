...
这里记录内部一个自助报表服务的实践. 办法就是把用户交互操作直接翻译到SQL:
...
- 时间字段: 对于详情表, 可能是原始时间戳, 如订单表, 对于报表而言, 一般是该行汇总到小时或者天维度
  - 默认必须要对时间字段进行筛选及聚合, 不考虑CRUD类型的需求
- 维度字段: 相关ID, 如产品, 用户, 销售等, 为了方便筛选, 一些冗余字段也要做进去, 如产品所属部门, 销售负责人等.
- 指标字段: 如`sales`, `incomes`, 默认通过`sum`汇总, 因为抛开维度谈`avg`/`max`等意义不大
- 虚拟字段, 可能会关联到同一行其他字段:
  - 虚拟维度字段:
    - 单行内转换的: `date(ts)`, `hour(ts)`, `case field when ... then ... end`, 等
    - 虚拟他表维度字段: 没有在表内冗余的字段, 要定义好关联关系, 查询时翻译为`IN (...)`
...
性能角度考虑, 不考虑生成JOIN表的逻辑. 跟据具体业务需求做冗余汇总报表.

一些简单的, 数量可控的关联查询, 通过翻译成事实表相关字段IN的逻辑实现.
...
对于MySQL里面的报表, 数据量大了很难一表走天下, 需要跟据不同汇总维度即筛选条件做聚合表, 跟据本次查询最终需要涉及的字段, 判定并返回一个最优的聚合表进行查询. 如:

- `aid_bid_country_stat` 最细粒度表
- `aid_country_stat` 解决不涉及bid字段的查询需求
- `country_aid_stat` 进一步优化`where country in ... group by aid`的需求
...
- 每次重新查询先看下查询数据版本是否变更, 如否, 直接返回上次缓存结果, 从而最大化的利用缓存, 且避免脏读问题
...
后续我们将大部分查询直接基于Clickhouse的大宽表进行, 更进一步减少了统计表汇总同步的工作.
